...
class DateTimeValidator extends FieldValidator {
    ...
    private function toStr(string $date) : bool|string
    {
        ...
        $str = sprintf('%s.%s.%s::%s.%s.%s',
            ...
            str_pad( ( (!!$parts['hour']) ? $parts['hour'] : 0 ) , 2, '0', STR_PAD_LEFT),
            str_pad( ( (!!$parts['minute']) ? $parts['minute'] : 0 ) , 2, '0', STR_PAD_LEFT),
            str_pad( ( (!!$parts['second']) ? $parts['second'] : 0 ) , 2, '0', STR_PAD_LEFT)
        );
    ...
    public function isBefore(string|DateTime $value, bool $equal = true) : self
    {
        ...
        {
            if($cmp > 0)
                return $this->registerError('datetime_before_and_equal', ['value' => $value]);
        }
        else
        {
            if($cmp >= 0)
                return $this->registerError('datetime_before', ['value' => $value]);
        }
    ...
    public function isAfter(string|DateTime $value, bool $equal = true) : self
    {
        ...
        {
            if($cmp > 0)
                return $this->registerError('datetime_after_and_equal', ['value' => $value]);
        }
        else
        {
            if($cmp >= 0)
                return $this->registerError('datetime_after', ['value' => $value]);
        }
    ...
    public function between(string|DateTime $min, string|DateTime $max, bool $equal = true) : self
    {
        ...
        if($equal)
        {
            if($this->str_value < $min_val || $this->str_value > $max_val)
                return $this->registerError('datetime_between_and_equal', ['min' => $min, 'max' => $max]);
        }
        else
        {
            if($this->str_value <= $min_val || $this->str_value >= $max_val)
                return $this->registerError('datetime_between', ['min' => $min, 'max' => $max]);
        }
    ...
    public function notBetween(string|DateTime $min, string|DateTime $max, bool $notEqual = true) : self
    {
        ...
        {
            if($this->str_value >= $min_val && $this->str_value <= $max_val)
                return $this->registerError('datetime_not_between_and_equal', ['min' => $min, 'max' => $max]);
        }
        else
        {
            if($this->str_value > $min_val && $this->str_value < $max_val)
                return $this->registerError('datetime_not_between', ['min' => $min, 'max' => $max]);
        }
