...
class YAMLParser
{
     ...
     *  Usage:
     *  <code>
     *   $array = YAMLParser::YAMLLoad('lucky.yml');
     *   print_r($array);
     *  </code>
     *
     * @return array
     * @param array $input Path of YAML file or string containing YAML
     */
    public static function YAMLLoad(array $input) : array
    {
        $parser = new YAMLParser();
     ...
     */
    public static function YAMLDump(array $array, int|bool $indent = false, int|bool $wordwrap = false)
    {
        $parser = new YAMLParser();

        return $parser->dump($array, $indent, $wordwrap);
    }
...
//        }
        ...
        foreach ($yaml as $linenum => $line)
        {
            ...
            {
                $error = sprintf('ERROR: %sLine %d in your input YAML begins with a tab. YAML only recognizes spaces. Please reformat.', (isset($file) ? "File $file " : ''), $linenum + 1);
            ...
            else if (!!$this->_inBlock && empty($ifchk))
            {
     ...
     * @param int|bool $wordwrap Pass in 0 for no wordwrap, false for default (40)
     */
    public function dump(array $array, int|bool $indent = false, int|bool $wordwrap = false) : string
    {
     ...
     */
    protected function _yamlize(string $key, mixed $value, $indent)
    {
        if (is_array($value))
        {
